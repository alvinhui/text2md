...
ENABLE_LOCAL_DOMAIN=0

if [[ "${1:-}" == "--with-local-domain" ]]; then
  ENABLE_LOCAL_DOMAIN=1
  shift
fi

if [[ $# -ne 0 ]]; then
  echo "用法:"
  echo "  bash scripts/start-random.sh"
  echo "  bash scripts/start-random.sh --with-local-domain"
  exit 1
fi
...
if [[ "${ENABLE_LOCAL_DOMAIN}" -eq 1 ]]; then
  echo "正在配置 http://md2text.local/ 域名映射（可能需要管理员权限）。"
  echo "如出现 Password 提示，请输入你的本机管理员密码。"
  sudo bash scripts/map-md2text-local.sh "${PORT}"
  echo "本地访问: http://md2text.local/"
fi
